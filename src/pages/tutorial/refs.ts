      ...
      content: () => {
        __.div({ class: "flex flex-col gap-4" }, () => {
          __.p(() => {
            ...
            __.code({ text: "ref" });
            __.text(
              ` method, save the result to a variable, and then pass it as the ref option to the element you want to access.`,
            );
          });
          __.p(() => {
            __.text(
            ...
            __.text(
              ` As you can see in the example, when we try to print out the value of the ref it is `,
            );
            __.code({ text: "null" });
            __.text(
              `. In the next section, let's look at how we can hook into the lifecycle of a component and access the ref after the component has been mounted.`,
            );
          });
        });
      },
